"""Shared PB-CWM preference/CEM composition for VBLRL-Adapt."""
...
class VBLRLAdaptOnline:
    """Keep Bayesian dynamics reward-free and use learned preferences for CEM."""
    ...
    def __init__(
        ...
        self.min_preferences_before_planning = int(preference_config["min_preferences_before_planning"])
        self.dynamics = VBLRLAdaptDynamicsLearner(
            obs_dim=obs_dim,
        ...
        planner_kwargs.update({
            ...
            "device": self.device,
            "candidate_keep_per_iteration": preference_config["candidate_keep_per_iteration"],
            "candidate_keep_final_elites": preference_config["candidate_keep_final_elites"],
            "dynamics_samples": vblrl_config["planning_model_samples"],
        })
    ...
    def bootstrap(self, num_queries: int, horizon: int, action_low: np.ndarray, action_high: np.ndarray) -> dict[str, float]:
        ...
        return {
            ...
            **self.reward_ensemble.update(self.preference_buffer, max(1, min(10, num_queries))),
        }

    def query_and_update(self, candidates: Sequence[TrajectorySegment], num_queries: int, reward_updates: int) -> dict[str, float]:
        ...
        return {
            ...
            **metrics,
            ...
            "ensemble_disagreement": float(np.mean([score for _, score in selected])) if selected else 0.0,
        }

    def generate_preference_examples(self, num_examples: int, horizon: int, action_low: np.ndarray, action_high: np.ndarray, seed: int) -> list[PreferenceExample]:
        starts = self.dynamics.seed_observations()
        if not starts:
            return []
    ...
    def _label_example(self, traj_a: TrajectorySegment, traj_b: TrajectorySegment) -> PreferenceExample | None:
        label = self.teacher.label(traj_a, traj_b)
        return None if label is None else PreferenceExample(traj_a, traj_b, label)

    def _random_rollout(self, start_obs: np.ndarray, horizon: int, action_low: np.ndarray, action_high: np.ndarray, rng: np.random.Generator | None = None) -> TrajectorySegment:
