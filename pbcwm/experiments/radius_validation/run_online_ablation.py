...
def run_online_ablation(stream_path: str | Path, *, variants: tuple[str, ...] = ("W1", "W2", "W3", "W4"), seed: int = 0, device: str = "cpu", max_steps: int | None = None) -> dict:
    ...
    results = [run_variant(stream, name, seed=seed, device=device, max_steps=max_steps) for name in variants]
...
def main() -> None:
    ...
    result = run_online_ablation(args.stream, device=args.device, max_steps=args.max_steps)
