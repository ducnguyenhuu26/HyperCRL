...
def build_evaluation_checkpoints(config: ProtocolConfig, schedule: LifetimeSchedule) -> tuple[EvaluationCheckpoint, ...]:
    points: dict[tuple[int, int | None], EvaluationCheckpoint] = {}
    for stage in schedule.stages:
        for fraction in config.evaluation.stage_fractions:
            ...
            points[(global_step, None)] = EvaluationCheckpoint(global_step, stage.segment_id, stage.dynamics_id, stage.visit_id, fraction)
        if stage.visit_id > 0:
            for interactions in config.evaluation.recurrence_interactions:
                ...
                if global_step <= stage.end_step:
                    points[(global_step, interactions)] = EvaluationCheckpoint(global_step, stage.segment_id, stage.dynamics_id, stage.visit_id, interactions / stage.realized_length, interactions)
